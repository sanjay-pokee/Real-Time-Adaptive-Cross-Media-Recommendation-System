...
\newcommand{\blk}[1]{{\scriptsize\textbf{#1}}\par\vspace{0.9mm}}

% --- Bulleted list with room to breathe ---
\newenvironment{tight}{\begin{itemize}\scriptsize\setlength{\itemsep}{3.2pt}}{\end{itemize}}
...
\begin{frame}
    ...
    \centering
    \IfFileExists{logo.png}{\includegraphics[width=2.4cm]{logo.png}\\[0.1cm]}{\vspace{0.3cm}}
...
% ---------------------------------------------------------------------
\begin{frame}{Previous Panel Review --- How Each Comment Was Addressed}
...
\begin{frame}{2. Implementation Progress ($\approx$75\%)}
...
\vspace{-1mm}
{\scriptsize\textbf{Stack:} React 19 $\rightarrow$ FastAPI + Pydantic v2 (8 endpoints) $\rightarrow$
SBERT + Qdrant + MySQL 8 + PyTorch. Solid arrows = request time; dashed = training time.\\[1mm]
\textbf{Two stages, in this order:} \textcolor{amritaMaroon}{\textbf{eligibility}} decides what may be
retrieved at all, then \textcolor{amritaTeal}{\textbf{relevance}} ranks what survived ---
$S = S_{sem} + \lambda_g S_{graph} + \lambda_e S_{ema} + \lambda_k S_{kg} + \lambda_p S_{profile}$,
with every sub-score returned to the UI.}
\end{frame}
...
\begin{frame}{2. Data Layer --- Sources and Scale on Disk}
...
\begin{tight}
  ...
  \item Lexical hits are capped at half the page, so an exact match never crowds out the
        semantic result set.
...
\end{columns}
\vspace{2mm}
{\scriptsize $S_{\mathit{sem}} = \cos(\mathbf{e}_i, \mathbf{q})$ over one shared 384-d space for
queries, items and user vectors --- so cosine applies with no projection layer. The limit of this
head: semantic similarity cannot express \emph{behaviour}, which is what the graph model adds.}
\end{frame}
...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
  inp/.style={rounded corners=9pt, draw=amritaSlate!80, fill=amritaSlate!10,
              align=center, minimum height=9mm, minimum width=2.6cm, font=\large},
  proc/.style={rounded corners=2pt, draw=amritaMaroon!75, fill=amritaMaroon!8,
              align=center, minimum height=10mm, minimum width=3.0cm, font=\large},
  gate/.style={rounded corners=2pt, draw=amritaMaroon, fill=amritaMaroon!22,
              align=center, minimum height=10mm, minimum width=3.0cm, font=\large},
  head/.style={rounded corners=2pt, draw=amritaTeal!80, fill=amritaTeal!7,
              align=center, minimum height=9mm, minimum width=4.9cm, font=\large},
  sum/.style={circle, draw=amritaMaroon, fill=amritaMaroon!15, minimum size=11mm, font=\LARGE},
  outp/.style={rounded corners=2pt, draw=amritaMaroon, fill=amritaMaroon!15,
              align=center, minimum height=10mm, minimum width=3.0cm, font=\large},
  fl/.style={-{Latex[length=2.6mm]}, draw=amritaSlate, line width=0.8pt},
  fw/.style={-{Latex[length=2.4mm]}, draw=amritaTeal!85, line width=0.7pt},
  fb/.style={-{Latex[length=2.6mm]}, draw=amritaAmber, line width=0.8pt, dashed},
  tag/.style={font=\normalsize\itshape, text=amritaAmber, anchor=west}
]
\node[inp] (q)  at (0,4.55) {query $q$};
\node[inp] (u)  at (0,3.40) {user $u$, age};
\node[inp] (kk) at (0,2.25) {$K$, domain};

\node[proc] (val) at (3.60,3.40) {Pydantic\\validation};
\draw[fl] (q) -- (val); \draw[fl] (u) -- (val); \draw[fl] (kk) -- (val);

\node[gate] (gate) at (7.40,3.40) {eligibility\\filter};
\draw[fl] (val) -- (gate);

\node[proc] (enc) at (11.20,4.65) {SBERT encode};
\node[proc] (cre) at (11.20,2.15) {lexical match};
\draw[fl] (gate) -- (enc); \draw[fl] (gate) -- (cre);
\node[proc, minimum width=3.2cm] (cand) at (15.00,3.40) {Qdrant ANN\\+ dedup};
\draw[fl] (enc) -- (cand); \draw[fl] (cre) -- (cand);

\node[head] (h1) at (20.50,5.65) {$S_{\mathit{sem}}=\cos(\mathbf{e}_i,\mathbf{q})$ \ \ $\times\,1.00$};
\node[head] (h2) at (20.50,4.53) {$S_{\mathit{graph}}=\mathbf{e}_u^{\top}\mathbf{e}_i$ \ \ $\times\,0.20$};
\node[head] (h3) at (20.50,3.41) {$S_{\mathit{ema}}=\cos(\mathbf{e}_i,\mathbf{u}_t)$ \ \ $\times\,0.15$};
\node[head] (h4) at (20.50,2.29) {$S_{\mathit{kg}}$ token overlap \ \ $\times\,0.08$};
\node[head] (h5) at (20.50,1.17) {$S_{\mathit{profile}}$ age / job \ \ $\times\,0.10$};
\foreach \h in {h1,h2,h3,h4,h5}{\draw[fw] (cand.east) -- (\h.west);}

\node[sum]  (sig) at (25.00,3.41) {$\Sigma$};
\foreach \h in {h1,h2,h3,h4,h5}{\draw[fw] (\h.east) -- (sig);}
\node[outp] (out) at (25.00,1.17) {sort $\rightarrow$ top-$K$};
\draw[fl] (sig) -- (out);

\draw[fb] (out.south) -- (25.00,0.30) -- (3.60,0.30) -- (val.south);
\node[tag] at (9.20,0.62) {real-time EMA feedback loop};
\end{tikzpicture}}
...
\begin{frame}{2. Constraint \& Eligibility --- Domain Packs and Audience}
...
% ---------------------------------------------------------------------
\begin{frame}{2. System in Action --- Audience Filtering}
\framesubtitle{The same query at age 8 and at age 25}
\vspace{1mm}
\begin{columns}[T]
\column{0.485\textwidth}
\begin{center}
\shot{age-child}{width=\linewidth,height=4.6cm,keepaspectratio}%
{Same query, age set to 8. Show the maturity badges and the reduced result count.}
\end{center}
\column{0.485\textwidth}
\begin{center}
\shot{age-adult}{width=\linewidth,height=4.6cm,keepaspectratio}%
{Same query, age set to 25. Show the wider result set, and on a health or finance query the advisory banner.}
\end{center}
\end{columns}
\vspace{2.5mm}
{\scriptsize\textbf{What to watch:} the result count changes, the maturity badge on each card
changes, and on a tier-1 domain an advisory banner appears above the results. No ranking code ran
differently --- the ineligible items were never retrieved.}
\end{frame}
...
% ---------------------------------------------------------------------
\begin{frame}{3. Is Each Module Justified?}
\framesubtitle{Why it matters $\cdot$ what breaks without it $\cdot$ is the current build defensible}
\vspace{0.5mm}
\begin{center}
{\tiny
\renewcommand{\arraystretch}{1.40}
\begin{tabular}{p{1.55cm} p{3.15cm} p{2.95cm} p{3.25cm}}
\toprule
\textbf{Module} & \textbf{Why it matters} & \textbf{What breaks without it} & \textbf{Current build defensible?}\\
\midrule
Data layer
 & One schema over 9 heterogeneous sources makes cross-domain ranking possible at all.
 & Every downstream module needs per-source special cases.
 & \textbf{Yes.} 120{,}747 rows, validated, 27 preprocessing tests.\\
Domain registry
 & Turns "add a vertical" from a code change into a config change.
 & The engine stays entertainment-shaped.
 & \textbf{Yes.} 4 packs live; risk tier and advisory drive real behaviour.\\
Embedding
 & One shared space lets a film and a finance book be compared with no learned bridge.
 & Cold-start items cannot be ranked at all.
 & \textbf{Yes, with a caveat.} MiniLM is small and general; a tuned encoder is Review 3.\\
Retrieval
 & Eligibility filtering happens inside the ANN search, not after it.
 & Post-filtering would let ineligible items be scored and promoted.
 & \textbf{Yes.} Hybrid lexical and vector, with the matched field exposed.\\
Audience
 & The safety property the panel asked for, enforced as a hard predicate.
 & Age filtering degrades to a demotion another head can undo.
 & \textbf{Yes.} 0.000 violations over 9 profiles; heuristics are the known limit.\\
EMA
 & Makes the "real-time adaptive" claim concrete.
 & Adaptation costs a retrain, so it is no longer real time.
 & \textbf{Yes.} One vector write per event; no session decay yet.\\
Graph CF
 & Behaviour that text cannot express; a reranker over the candidate set.
 & Only content similarity remains; no collaborative signal.
 & \textbf{Partly.} Benchmarks are real on 3 domains; the served artifact is not yet.\\
Knowledge graph
 & Cross-domain bridge and exact-entity matching at zero parameters.
 & Named-entity queries smear; cold items lose their structural signal.
 & \textbf{Yes, as scoped.} Token overlap at runtime; the graph database is Review 3.\\
Persistence
 & Interactions must survive a restart for the EMA loop to mean anything.
 & The feedback loop resets on every process restart.
 & \textbf{Yes.} Validated, idempotent, indexed on the hot path.\\
API + UI
 & Every sub-score is returned and drawn, so a recommendation is explainable.
 & The system becomes a black box.
 & \textbf{Yes.} 8 endpoints, 19 components, score breakdown per card.\\
\bottomrule
\end{tabular}}
\end{center}
\vspace{1mm}
{\scriptsize\textbf{Added in integration since Review 1:} the eligibility filter compiled into the
search query $\cdot$ the domain registry driving API, filter and UI from one file $\cdot$ the
matched field surfaced end to end $\cdot$ risk-tier advisories rendered from backend metadata
$\cdot$ EMA writes inside the interaction transaction.}
\end{frame}

% ---------------------------------------------------------------------
\begin{frame}{3. Comparison with Existing Solutions}
\framesubtitle{Where this sits against production systems and published baselines}
\vspace{1mm}
\begin{center}
{\scriptsize
\renewcommand{\arraystretch}{1.45}
\begin{tabular}{p{2.9cm} c c c c}
\toprule
...
\midrule
Netflix / Prime (in-domain CF)  & No     & Weak & Batch   & Ratings only\\
Spotify (audio + CF)            & No     & Weak & Session & Explicit flag\\
Amazon item-to-item CF          & Partly & No   & Batch   & Category\\
...
\begin{itemize}\tiny\setlength{\itemsep}{3pt}
  \item \textbf{One query spans four unrelated domains.} Every system above is single-domain by
        construction --- a Netflix query cannot return a finance book. Ours can, because the
        namespaces are shared rather than bridged.
  \item \textbf{Eligibility is a hard pre-filter, not a ranking penalty.} The systems above filter
        on metadata \emph{after} retrieval; ours cannot retrieve an ineligible item at all.
  \item \textbf{Adding a vertical is a config change} --- no retraining, no new code path.
  \item \textbf{Every sub-score is returned.} Five heads, visible per card.
\end{itemize}
...
\begin{itemize}\tiny\setlength{\itemsep}{3pt}
  ...
  \item \textbf{The served LightGCN artifact is synthetic.} The benchmarks are real; the
        deployment is not yet.
  \item \textbf{No latency study.} P95 under load is unmeasured.
\end{itemize}
\vspace{1.5mm}
...
\begin{frame}{Appendix --- Scenario Evaluation Across All Four Domains}
...
\begin{frame}{Appendix --- Known Limitations and Next Steps}
