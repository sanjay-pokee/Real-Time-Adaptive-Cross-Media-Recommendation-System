...
% LightGCN numbers, written from reports/benchmarks/*.json so they cannot drift
% from the Kaggle run that produced them. See reports/domain_benchmark_table.md.
% Movies_and_TV (entertainment):
\newcommand{\mvRecall}{0.0288}
\newcommand{\mvNDCG}{0.0125}
\newcommand{\mvPop}{0.0182}
% Industrial_and_Scientific (industry):
\newcommand{\inRecall}{0.0326}
\newcommand{\inNDCG}{0.0138}
\newcommand{\inPop}{0.0264}
% Health_and_Household (health) -- the domain popularity wins:
\newcommand{\hhRecall}{0.0121}
\newcommand{\hhNDCG}{0.0046}
\newcommand{\hhPop}{0.0221}
% ---------------------------------------------------------------------
...
\begin{frame}
    ...
    \centering
    \IfFileExists{logo.png}{\includegraphics[width=2.6cm]{logo.png}\\[0.05cm]}{\vspace{0.2cm}}

    {\large \textbf{\textcolor{amritaMaroon}{Real-Time Adaptive Cross-Media Recommendation System}}} \\
    \vspace{0.04cm}
    {\scriptsize \textit{A Domain-Agnostic Recommendation Substrate with Pluggable Domain Packs
     and an Audience Constraint Layer ---\\ Entertainment, Health, Industry and Finance}}\\
    \vspace{0.10cm}
...
\node[cap] at (0.75,7.44) {React 19 $\cdot$ Vite $\cdot$ TailwindCSS $\cdot$ Framer Motion $\cdot$ axios \ \ (19 components, 2 routed pages)};
...
\node[box, minimum width=9.40cm, minimum height=9mm, font=\tiny\ttfamily] (api) at (5.61,5.24)
  {POST /recommend \ \ POST /recommend/item \ \ POST /interactions \ \ GET /domains\\
   GET /search/suggest \ \ GET /interactions/\{user\}/\{item\} \ \ GET /health \ \ GET /};
...
\node[store, minimum width=3.00cm] (qd) at (2.55,1.48) {Qdrant \texttt{content}\\\tiny 106{,}332 $\times$ 384 vectors, HNSW};
...
\node[offb] (o1) at (13.40,6.85) {\textbf{Raw datasets}\\TMDb API $\cdot$ Google Books\\Spotify Songs $\cdot$ Amazon Reviews 2023};
\node[offb] (o2) at (13.40,5.65) {\texttt{preprocessing.}\\\texttt{build\_content\_catalog}\\unified 20-column schema};
...
\begin{frame}{2. Implementation Progress ($\approx$75\%) \& Module Functionality}
...
\begin{table}
    ...
    \begin{tabular}{p{1.95cm} p{6.75cm} p{1.45cm} c}
        ...
        \midrule
        Data pipeline & \textbf{4 domains / 6 content types} normalised into one 20-column catalog: \textbf{106{,}332} items (28{,}352 music / 22{,}116 movies / 20{,}000 health / 20{,}000 industrial / 15{,}139 books / 725 finance) & \texttt{content\_catalog.csv} & \sOK \\
        Domain packs & Domain, maturity, minimum age and risk tier declared per content type in one config; a new vertical is a config entry, not a code change & \texttt{config/domains.yaml} & \sOK \\
        Embeddings & SBERT \texttt{all-MiniLM-L6-v2}, \textbf{106{,}332 $\times$ 384} float32; SHA-256 hash skips unchanged rows on rebuild & \texttt{content\_embeddings.npy} & \sOK \\
        Vector search & Qdrant collection with full payloads; cosine ANN plus optional \texttt{content\_type} and domain filters inside the search & \texttt{/recommend} 200 OK & \sOK \\
        ...
        Database layer & MySQL 8: 3 tables, FK + indices, idempotent upserts, batched catalog load, in-place collation migration & \texttt{backend/mysql\_store.py} & \sOK \\
        Audience filter & Eligibility layer on every result path: viewer age, safe mode, domain scope and risk-tier ceiling; fails closed on absent or corrupt metadata & \texttt{backend/audience.py} & \sOK \\
        Content ratings & Real board certifications fetched per title (TMDb US) drive movie maturity; a genre may restrict a row but never relax one & \texttt{tmdb\_certifications.csv} & \sOK \\
        ...
        KG scoring & Entity overlap between query terms and item title / description / creators / categories, $\lambda_k{=}0.08$; visualizer shows the target typed schema & \texttt{backend/knowledge\_graph.py} & \sOK \\
        Explainability & Per-item signal breakdown plus a counterfactual panel: which signal, removed, would have changed the rank & \texttt{ExplainPanel.jsx} & \sOK \\
        Backend APIs & \textbf{8} FastAPI endpoints, Pydantic v2 validation, typed error mapping (400 / 503), OpenAPI docs & \texttt{/docs} & \sOK \\
        Frontend UI & React 19 SPA: \textbf{19} components, 2 routed pages, domain and audience controls, score bars, similar-item drawer, feedback & \texttt{npm run build} & \sOK \\
        Integration & Loop closed: UI feedback $\rightarrow$ MySQL $\rightarrow$ EMA update $\rightarrow$ ranking shifts, with no retraining & live demo & \sOK \\
        Quality gate & \textbf{195 / 195} unit + integration tests green in \textbf{$<$5 s} across 9 suites & \texttt{pytest -q} & \sOK \\
        \midrule
        Benchmark eval & Amazon Reviews 2023 pipeline on a Kaggle T4: \textbf{Movies\_and\_TV}, \textbf{Health\_and\_Household} and \textbf{Industrial\_and\_Scientific} all trained against a popularity control & \texttt{reports/benchmarks/} & \sOK \\
        Remaining work & Neo4j typed KG + traversal scoring, Docker Compose + CI/CD, P95 latency benchmarking, diversity rerank, finance-domain benchmark, user study & Review 3 & \sPLN \\
        \bottomrule
    \end{tabular}
\end{table}
\vspace{-1mm}
{\scriptsize \textbf{Demonstration evidence:} live UI walkthrough across all four domains $\cdot$ the same query at age 8 and age 25 $\cdot$
Swagger \texttt{/docs} request--response $\cdot$ \texttt{pytest} run $\cdot$ interactive KG visualizer $\cdot$ \textbf{47} commits on \texttt{main}.}
\end{frame}

% ---------------------------------------------------------------------
\begin{frame}{2. Model Design -- Multi-Signal Ranking Pipeline}
\framesubtitle{How one query becomes an explainable, eligibility-filtered ranked list spanning four domains}
\vspace{-2mm}
\begin{center}
\begin{tikzpicture}[
...
{\scriptsize \textbf{Shipping today: entity-overlap scoring}}
\begin{itemize}\tiny
  \item The runtime signal is \textbf{token-set overlap} between query terms and each item's title, description, creators and categories --- the $S_{\mathit{kg}}$ below --- contributing $\lambda_k{=}0.08$ to the blend.
  \item No traversal, and no materialised node/edge store in the request path: the score is computed from the catalog row itself, which is why it is defined for items with zero interactions.
\end{itemize}
\vspace{0.5mm}
{\scriptsize \textbf{Target typed schema (\texttt{kg\_visualizer.html})}}
\begin{itemize}\tiny
  \item \textbf{564 nodes / 845 edges} in the visualizer's worked example: 233 \emph{Creator}, 125 \emph{Song}, 74 \emph{Genre}, 68 \emph{Book}, 58 \emph{Movie}, 6 \emph{User}; edges \texttt{HAS\_GENRE} (498), \texttt{CREATED\_BY} (319), \texttt{AFFINITY} (10), \texttt{RECOMMENDED}.
  \item Migrating this to \textbf{Neo4j with traversal-based scoring} is the Review-3 deliverable; the visualizer is the schema we are building to, not a runtime graph.
\end{itemize}
\vspace{0.5mm}
{\scriptsize \textbf{Technical role in the pipeline}}
\begin{itemize}\tiny
  \item \textbf{Cross-domain bridge.} Creator / genre / profile vocabularies are \emph{shared}, so the same overlap score connects a \emph{Movie} query to a \emph{Book} with no domain-specific rule.
  ...
  \item \textbf{Safety is no longer a weight.} Age suitability was a soft negative weight ($w_-\!=\!0.25$) that merely demoted unsuitable categories. It is now a \emph{hard eligibility filter} applied before ranking --- see the next frame.
...
% ---------------------------------------------------------------------
\begin{frame}{2. Domain Generality \& the Audience Constraint Layer}
\framesubtitle{Responding to Review-1 feedback: a domain-agnostic substrate, not an entertainment-only recommender}
\vspace{-1mm}
\begin{columns}[T]
\column{0.48\textwidth}
{\scriptsize \textbf{(a) Pluggable domain packs}}
\begin{itemize}\tiny
  \item Four domains over six content types, all declared in \texttt{config/domains.yaml}: \textbf{entertainment} (movie, book, music), \textbf{health}, \textbf{industry}, \textbf{finance}.
  \item A pack declares its \texttt{domain}, \texttt{default\_maturity}, \texttt{maturity\_source} and \texttt{risk\_tier}. Adding a vertical is a config entry plus a normalizer --- the retrieval, ranking and audience layers are unchanged.
  \item \textbf{Framing.} Health and finance are scoped to \emph{information and product discovery}. The engine recommends content and products; it does not diagnose, prescribe, or give investment advice, and carries no clinical or regulatory validation.
\end{itemize}
\vspace{1mm}
{\scriptsize \textbf{(b) Eligibility, not demotion}}
\begin{itemize}\tiny
  \item Four independent constraints, applied \emph{before} ranking: viewer age vs item minimum age, safe mode, domain scope, and a risk-tier ceiling that withholds professional-advice items.
  \item \textbf{Fails closed.} An undeclared age caps at the teen ceiling; an unrecognised content type is gated; a corrupt or absent maturity value resolves to the \emph{most} restrictive level, not the least.
  \item When the filter withholds everything, the UI says \emph{why} rather than rendering an empty list.
\end{itemize}

\column{0.48\textwidth}
{\scriptsize \textbf{(c) Measured constraint enforcement}}\\
{\tiny \hspace{1mm} \texttt{scripts/evaluate\_constraints.py --k 20}}
\vspace{-1mm}
\begin{table}
\centering
\tiny
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{p{1.55cm} c c c}
\toprule
\textbf{Viewer} & \textbf{Coverage} & \textbf{Viol.} & \textbf{Viol.} \\
 & \textbf{of catalog} & \textbf{on} & \textbf{off} \\
\midrule
Child (8)        & 3.5\%   & \textbf{0.000} & 1.000 \\
Teen (15)        & 52.0\%  & \textbf{0.000} & 1.000 \\
Adult (25)       & 100\%   & \textbf{0.000} & 0.000 \\
Adult, safe mode & 3.5\%   & \textbf{0.000} & 1.000 \\
Age not supplied & 52.0\%  & \textbf{0.000} & 1.000 \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-2mm}
{\tiny Nine profiles measured in total, including one per domain; every one enforces at
\textbf{0.000}. \emph{Overlap@20} with the unconstrained ranking is \textbf{0\%} for a child and
\textbf{100\%} for an adult --- the filter is doing real work for the young viewer and costs the
adult nothing.}
\vspace{1.5mm}

{\scriptsize \textbf{(d) A genre is not a rating}}
\begin{itemize}\tiny
  \item Movie maturity now comes from the \textbf{real TMDb US board certification} per title, fetched into \texttt{tmdb\_certifications.csv}: \textbf{14{,}024} of 22{,}116 movies carry a board rating (R 7{,}303 / PG-13 3{,}518 / PG 2{,}277 / G 834 / NC-17 92).
  \item Previously it was inferred from genre text, where \texttt{Animation} read as child-friendly --- so \emph{Akira}, \emph{Heavy Metal}, \emph{Grave of the Fireflies} and \emph{Waltz with Bashir} were all rated \texttt{all\_ages}.
  \item A genre may now \emph{restrict} a title but never \emph{relax} one, and \texttt{NR} is treated as "no board rated this", not as harmless.
\end{itemize}
\end{columns}
\end{frame}
...
\framesubtitle{Scenario-level metrics $\cdot$ cross-domain LightGCN holdout $\cdot$ constraint enforcement}
\vspace{-1mm}
\begin{columns}[T]
\column{0.55\textwidth}
{\scriptsize \textbf{(a) Measured prototype evaluation}}\\
{\tiny \hspace{1mm} 8 scenarios $\times$ demo profiles --- \emph{one per domain} ---
via \texttt{scripts/evaluate\_recommendation\_table.py}}
\vspace{-1mm}
\begin{table}
\centering
\tiny
\renewcommand{\arraystretch}{1.10}
\begin{tabular}{p{1.85cm} p{1.85cm} c c c}
\toprule
\textbf{Scenario} & \textbf{Profile} & \textbf{Person} & \textbf{Profile} & \textbf{Avg.} \\
 & & \textbf{Hit@5} & \textbf{Align@5} & \textbf{KG} \\
\midrule
Actor / director & \texttt{user\_scifi} & 5/5 & 5/5 & 0.667 \\
Singer / artist & \texttt{user\_music\_pop} & 5/5 & 5/5 & 0.667 \\
Profession-aware & \texttt{user\_books\_learning} & --- & 5/5 & 1.000 \\
Family-safe & \texttt{user\_family} & --- & 5/5 & 1.000 \\
Cross-media intent & \texttt{user\_scifi} & --- & 5/5 & 1.000 \\
Health discovery & \texttt{user\_health\_...} & --- & 5/5 & 1.000 \\
Industrial supply & \texttt{user\_industry\_...} & --- & 5/5 & 0.900 \\
Finance literacy & \texttt{user\_finance\_...} & --- & 5/5 & 0.900 \\
\midrule
\textbf{Aggregate} & & \textbf{1.00} & \textbf{1.00} & \textbf{0.892} \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-2mm}
{\tiny Average profile alignment \textbf{0.957}; DomainCoverage@10 \textbf{0.50} --- 4 of 8 scenarios
returned more than one content type in the top-10, and the four that did not are the
single-vertical queries where that is the correct answer.
Representative top-1 hits: \emph{Avatar: Fire and Ash} (James Cameron), \emph{Lover (Remix)}
(Taylor Swift), \emph{Monsters vs Aliens} (space adventure), \emph{OmegaXL Joint Support}
(health), \emph{Intuit Cash Register} (finance).}
...
\toprule
\textbf{Domain} & \textbf{R@20} & \textbf{N@20} & \textbf{Pop.} & \textbf{Lift} \\
\midrule
Entertainment & \textbf{\mvRecall} & \mvNDCG & \mvPop & \textbf{1.59$\times$} \\
Industry & \textbf{\inRecall} & \inNDCG & \inPop & \textbf{1.24$\times$} \\
Health & \hhRecall & \hhNDCG & \textbf{\hhPop} & 0.55$\times$ \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-2mm}
{\tiny \textbf{Protocol.} Ratings $\geq 4$ binarised to implicit positives; iterative 10-core on
users and items; leave-last-out holdout per user; 50k-user sample; 60 epochs on a Kaggle T4;
...
\emph{LightGCN $>$ popularity}, not a large absolute value.
Industrial \& Scientific is an order of magnitude smaller (413k raw interactions vs 7.4M) and
does not survive a 10-core filter, so it uses the source file's 5-core floor.}
\vspace{1mm}

{\tiny \textbf{LightGCN loses on health, and that is the result.} Household goods are commodity
purchases where nearly everyone buys the same top sellers, so a popularity ranker is genuinely
hard to beat; entertainment is taste-driven, where collaborative signal pays. This is the
empirical case for the engine being a \emph{hybrid} --- semantic retrieval and the constraint
layer carry the verticals where graph CF is weak, and \texttt{lightgcn\_weight} is the knob
that expresses it per domain.}
\vspace{1.5mm}

{\scriptsize \textbf{(c) Engineering quality gate}}
\begin{itemize}\tiny
  \item \textbf{195 / 195} unit + integration tests green in under \textbf{5 s} across 9 suites.
  \item Catalog rebuild is idempotent and hash-cached; the embedding rebuild skips unchanged rows.
  \item Constraint enforcement is itself measured: \textbf{0.000} violation rate at $k{=}20$ across
        9 viewer profiles, against \textbf{1.000} with the filter disabled.
\end{itemize}
\end{columns}
\vspace{-1mm}
{\tiny \textcolor{amritaSlate}{All three domains reuse the identical pipeline and notebook --- only the
\texttt{CATEGORY} constant changes. Finance has no Amazon Reviews category of its own, so it carries
no LightGCN row; its 725 items are served by semantic retrieval and the constraint layer, and a
benchmark on an external finance dataset is Review-3 work.}}
\end{frame}
...
\begin{itemize}\tiny
  ...
  \item Global vs. source IDs prevent primary-key collisions across the four datasets; both id columns are 	exttt{utf8mb4\_bin}, because the database default collation is case-insensitive and Google Books ids are not.
\end{itemize}
...
\begin{itemize}\tiny
  ...
  \item \textbf{195} pytest cases over 9 suites; Uvicorn access / error logs captured; \texttt{oxlint} on the frontend.
